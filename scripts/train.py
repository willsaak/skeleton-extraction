...
@click.option('--checkpoint', type=click.Path(dir_okay=True), default='')
def main(inputs_path, targets_path, dst, epochs, batch_size, val_size, checkpoint):
    ...
    train_inputs = [load_image(x)[..., np.newaxis] for x in train_input_files]
    train_targets = [load_image(x)[..., np.newaxis] for x in train_target_files]
    val_inputs = [load_image(x)[..., np.newaxis] for x in val_input_files]
